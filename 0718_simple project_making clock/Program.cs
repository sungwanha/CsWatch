...
public class TimeObject
{
    ...
    public void Restart()
    {
        if (isRunning)                                                          // 스톱워치가 작동 중인 경우에만 실행
        {
            Console.WriteLine("스톱워치 이미 작동 중 재시작할 수 없습니다.");    // 이미 작동 중인 경우 메시지 출력
        }
        else
        {
            startTime = DateTime.Now - elapsedTime;             // 시작 시간 = 현재 시간 - 경과 시간
            isRunning = true;                                   // 스톱워치를 작동 중으로 설정
            Console.WriteLine("스톱워치 재시작");              // 재시작을 알리는 메시지 출력
        }
    }
...
class Program
{
    static void Main(string[] args)
    {
        ...
        while (true)                                // 무한 반복
        {
            Console.Write("명령을 입력해주세요 (start, stop, record, restart, reset): ");   // 사용자로부터 명령 입력 요청
            ...
            switch (command)                                            // 명령에 따라 분기
            {
                ...
                case "record":
                    ...
                    break;
                case "restart":
                    stopwatch.Restart();  // 스톱워치 재시작 메서드 호출
                    break;
                case "reset":
